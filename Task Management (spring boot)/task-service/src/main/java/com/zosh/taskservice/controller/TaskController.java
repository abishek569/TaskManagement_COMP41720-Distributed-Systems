...
//        List<Task> tasks = taskService.assignedUsersTask(user.getId(),status, sortByDeadline, sortByCreatedAt);
        List<Task> tasks = taskService.assignedUsersTask(user.getId(),TaskStatus.valueOf(status), sortByDeadline, sortByCreatedAt);
...
//        List<Task> tasks = taskService.getAllTasks(status, sortByDeadline, sortByCreatedAt);
        List<Task> tasks = taskService.getAllTasks(TaskStatus.valueOf(status), sortByDeadline, sortByCreatedAt);
        return new ResponseEntity<>(tasks, HttpStatus.OK);
    }
